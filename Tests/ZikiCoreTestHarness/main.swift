...
private func testHistoryPolicySearchesCaseInsensitively() throws {
    ...
    try expect(
        DictationHistoryPolicy.matching([matching, other], query: "ziki"),
        equals: [matching],
...
private func withTemporaryHistoryFile(
    ...
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("ziki-history-\(UUID().uuidString)")
...
private func testDiagnosticsStorePersistsAudioAndPipelineStages() throws {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("ziki-diagnostics-\(UUID().uuidString)")
    defer { try? FileManager.default.removeItem(at: directory) }
...
private func testSystemPastePolicyBlocksOnlyExplicitUnsafeTargets() throws {
    try expect(
        ...
        "Ziki must not paste into itself"
    )
    try expect(
        SystemPastePolicy.decide(
            focusedProcessIsOwnApp: false,
...
private func testAppUpdatePolicySelectsNewerSignedPackage() throws {
    ...
    let oldBrandRelease = Data(String(decoding: release, as: UTF8.self)
        .replacingOccurrences(of: "Ziki-", with: "Sotto-").utf8)
    do {
        _ = try AppUpdatePolicy.resolve(
            releaseData: oldBrandRelease,
            currentVersion: "0.2.13",
            architecture: "arm64"
        )
          ...
          "assets": [
            {
              ...
              "browser_download_url": "https://example.com/Ziki-0.2.14-macOS-arm64.zip"
            }
          ]
        }
        """.utf8
...
@main
private enum ZikiCoreTestHarness {
    static func main() {
